...
const InventoryEdit = forwardRef(({ inventoryToEdit, formType, className, warehouses }, ref) => {
  ...
  return (
    <form ref={ref} className={`inventoryEditForm ${className}`}>
      ...
      {fieldsToRender.map((field, index) => {
        ...
        if (field.name === "quantity") {
          return (
            <div key={index} className={`${selectedStatus === "out_of_stock" ? "hide-quantity" : ""}`}>
              <h3 className="font-H3-label inventoryEditForm__field-title">{field.title}</h3>
              <input className={`${field.value === "" ? "editNoData" : ""} inventoryEditForm__input-placeholder inventoryEditForm__input-placeholder--shrink`} name={field.name} value={formData.quantity} onChange={handleChange} />
              {hasError && (
        ...
        if (field.name === "category") {
          return (
            <div key={index}>
              <h3 className="font-H3-label inventoryEditForm__field-title">{field.title}</h3>
              <select name="category" value={field.value} onChange={handleChange} className={`inventoryEditForm__input-placeholder inventoryEditForm__input-placeholder--dropDown ${field.value === "" ? "editNoData" : ""}`}>
        ...
        if (field.name === "warehouse") {
          return (
            <div key={index}>
              <h3 className="font-H3-label inventoryEditForm__field-title">{field.title}</h3>
              <select name="warehouse" onChange={handleChange} className="inventoryEditForm__input-placeholder inventoryEditForm__input-placeholder--dropDown">
        ...
        if (field.name === "description") {
          return (
            <div key={index}>
              <h3 className="font-H3-label inventoryEditForm__field-title ">{field.title}</h3>
              <textarea className={`font-P2-BodyMedium inventoryEditForm__input-placeholder inventoryEditForm__description ${field.value === "" ? "editNoData" : ""}`} name={field.name} defaultValue={field.value} onChange={handleChange} required></textarea>
              {hasError && (
                <div className="editError__container">
                  <img src={errorImage} alt="error" className="error-image" />
                  <p className="editError__message font-P2-BodyMedium">{errorMessage}</p>
        ...
        return (
          <div key={index}>
            <h3 className="font-H3-label inventoryEditForm__field-title">{field.title}</h3>
            <input className={`inventoryEditForm__input-placeholder ${field.value === "" ? "editNoData" : ""} ${hasError ? "error" : ""}`} name={field.name} defaultValue={field.value} onChange={handleChange} required />
            {hasError && (
